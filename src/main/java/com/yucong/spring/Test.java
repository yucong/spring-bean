 ...
 * 2:应用上下文（由org.springframework.context.ApplicationContext接口定义）
 ...
 * 与ApplicationContext相比，有如下几点不同：
 * 
 * 1.BeanFactory容器中，不会调用ApplicationContextAware接口的setApplicationContext()方法。<br/>
 * 2.BeanPostProcessor接口的postProcessBeforeInitialization方法
 *   和postProcessAfterInitialization方法不会自动调用，必须自己通过代码手动注册
 * 
 * 3. BeanFactory容器启动的时候，不会去实例化所有bean，
 *   包括所有scope为singleton且非延迟加载的bean也是一样，而是在调用的时候去实例化。
 * 
 */
public class Test {
